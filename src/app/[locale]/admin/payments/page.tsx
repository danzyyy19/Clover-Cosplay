    ...
    return (
        <AdminLayout pageTitle="Payments">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px", marginBottom: "24px" }}>
                {[
                    { label: "Pending", value: payments.filter(p => p.status === "PENDING").length, color: "#F0E68C" },
                    { label: "Approved", value: payments.filter(p => p.status === "APPROVED").length, color: "#7FFFD4" },
                    { label: "Rejected", value: payments.filter(p => p.status === "REJECTED").length, color: "#FFB6C1" },
                ].map((stat) => (
            ...
            <div style={{
                ...
                <div style={{ paddingBottom: "20px", borderBottom: "1px solid var(--border-color)", marginBottom: "20px" }}>
                    <h3 style={{ fontSize: "16px", fontWeight: "600", color: "var(--text-primary)" }}>Payment Proofs</h3>
                </div>
