...
export default async function CustomerDashboardPage({
    ...
    const totalBookings = bookingStats.reduce((acc, curr) => acc + curr._count.id, 0);
    const activeRentals = bookingStats.find(s => s.status === "ACTIVE")?._count.id || 0;
    const pendingPayments = bookingStats.find(s => s.status === "PENDING")?._count.id || 0;
    const completedRentals = bookingStats.find(s => s.status === "COMPLETED")?._count.id || 0;
