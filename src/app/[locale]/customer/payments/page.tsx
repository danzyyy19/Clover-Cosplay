    ...
    return (
        <CustomerLayout pageTitle={pageTitle}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "14px", marginBottom: "24px" }}>
                {[
                    { label: locale === "th" ? "รอตรวจสอบ" : "Pending", value: payments.filter(p => p.status === "PENDING").length, color: "#F0E68C", icon: Clock },
                    { label: locale === "th" ? "อนุมัติแล้ว" : "Approved", value: payments.filter(p => p.status === "APPROVED").length, color: "#7FFFD4", icon: CheckCircle },
                    { label: locale === "th" ? "ถูกปฏิเสธ" : "Rejected", value: payments.filter(p => p.status === "REJECTED").length, color: "#FFB6C1", icon: XCircle },
                ].map((stat) => {
